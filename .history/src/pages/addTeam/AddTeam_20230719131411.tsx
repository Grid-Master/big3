...
const AddTeam: FC = () => {
  ...
  const { name, division, conference, foundationYear, imageUrl } = useAppSelector(
  ...
  const submitHandler: SubmitHandler<any> = async (data: Omit<Omit<ITeam, 'id'>, 'imageUrl'>) => {
    if (selectedImage) {
      ...
      if (id) {
        //@ts-ignore
        await dispatch(updateTeam({ ...data, imageUrl: imageResponse.payload, id }));
        alert('Team was updated');
      } else {
        //@ts-ignore
        await dispatch(addTeam({ ...data, imageUrl: imageResponse.payload }));
