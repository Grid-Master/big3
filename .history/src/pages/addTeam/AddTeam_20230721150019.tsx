...
import { useForm, SubmitHandler, FormProvider, useController, Controller } from 'react-hook-form';
...
const AddTeam: FC = () => {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    imageAdder.current!.value = ''; // Сброс значения input для возможности выбора одного и того же файла
    return fileList;
  };
            ...
            render={({ field: { ref, ...inputProps } }) => (
              <>
                <input
                  ...
                  ref={(e) => {
                    //@ts-ignore
                    ref.current = handleFileChange(e); // Устанавливаем значение файла через ref
                    //@ts-ignore
                    imageAdder.current = e; // Дублируем ref для fileInputRef, если нужно обращаться по другому имени
                  }}
                  {...inputProps}
                  type="file"
                  accept=".png"
